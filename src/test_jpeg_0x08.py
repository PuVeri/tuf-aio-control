#!/usr/bin/env python3
"""Preview or perform one explicitly armed ASUS LCD JPEG transfer."""
...
def build_transfer_segments(jpeg: bytes) -> tuple[TransferSegment, ...]:
    ...
    _validate_transfer_invariants(jpeg, result)
    return result
...
def _validate_transfer_invariants(
    ...
    if len(segments) != segment_count(len(jpeg)) or len(segments) > MAX_SEGMENTS:
...
def _select_target() -> tuple[HidrawInterface | None, str]:
    ...
    error = _device_validation_error(matches[0])
...
def _validate_open_target(fd: int, expected: HidrawInterface) -> None:
    ...
    expected_error = _device_validation_error(expected)
    ...
    current_matches = [
        ...
        and _device_validation_error(device) is None
    ]
...
def _print_preview(
    ...
    for segment in segments:
        print(
            f"  Segment {segment.index} Control: "
            f"{segment.control.hex(' ')}"
        )
    if device is None:
...
def _run_once(
    ...
    if not os.access(device.device_path, os.W_OK):
        print(
            "FEHLER: Keine Schreibberechtigung; Rechte wurden nicht verändert.",
            file=sys.stderr,
        )
    ...
    try:
        for segment in segments:
            try:
                ...
                print(
                    f"FEHLER vor Segment {segment.index}: "
                    f"Sicherheitsprüfung fehlgeschlagen: {error}",
                    file=sys.stderr,
                )
            ...
            try:
                ...
                print(
                    f"FEHLER: Write für Segment {segment.index} fehlgeschlagen: {error}",
                    file=sys.stderr,
                )
                return EXIT_IO_ERROR
            if written != HIDRAW_WRITE_BYTES:
                print(
                    f"FEHLER: Short Write bei Segment {segment.index}: "
        ...
        print(
            f"ERFOLG: Genau {len(segments)} Segmente einmalig geschrieben; "
        ...
        try:
            ...
            print(f"WARNUNG: Descriptor-Close fehlgeschlagen: {error}", file=sys.stderr)
        else:
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        ...
        description=(
            ...
            "und baut den festen ASUS-0x08-Transfer."
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="den später separat autorisierten Einmaltransfer ausdrücklich starten",
    )
    ...
    _print_preview(jpeg_path, digest, info, segments, device, device_status)
    ...
    if args.dry_run or not args.i_understand_the_risk:
        ...
        if not args.dry_run:
            print("Für einen späteren echten Lauf wäre --i-understand-the-risk nötig.")
        return EXIT_SUCCESS
